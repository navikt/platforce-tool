...
const TRASH_SVG = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none"
...
function renderRepo(repoView, scanMap, container) {
    ...
    const untrackedBadge =
        untrackedCount > 0
            ? `<span class="badge untracked">${untrackedCount} UNTRKD</span>`
            : "";
...
function renderTargets(data) {
    ...
    targetState = {
        ...
        transientDependencies: new Set(data.transientDependencies || []),
        drafted
    };
    ...
    if (security.status === "VULNERABLE") {
        ...
        if (security.vulnerableDependencies?.length) {
            security.vulnerableDependencies.forEach(vulnerable => {
                ...
                const suggestion = vulnerable.suggestedVersion;
